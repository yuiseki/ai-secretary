...
restore_window_to_front() {
  ...
  # system_normal_mode can leave these PWAs minimized. Clear the hidden state,
  # then raise/focus each window so the tiled wall becomes visible again.
...
declare -A DESKTOP_PATHS=(
  ["marinetraffic-com"]="${DESKTOP_DIR}/chrome-eiggboehmioopkkmepiijdhicjfokacg-Default.desktop"
  ["monitor-the-situation"]="${DESKTOP_DIR}/chrome-fpjghcebmepbfdchhgkapbjfaibonnee-Default.desktop"
  ["glint-trade"]="${DESKTOP_DIR}/chrome-lidakfpkjaoncppkjikdmmephmoangjc-Default.desktop"
  ["flightradar24-com"]="${DESKTOP_DIR}/chrome-bdchbohmgmandnkikanoblpemggpldff-Default.desktop"
)

declare -A WM_CLASSES=(
  ["marinetraffic-com"]="crx_eiggboehmioopkkmepiijdhicjfokacg.Chromium"
  ["monitor-the-situation"]="crx_fpjghcebmepbfdchhgkapbjfaibonnee.Chromium"
  ["glint-trade"]="crx_lidakfpkjaoncppkjikdmmephmoangjc.Chromium"
  ["flightradar24-com"]="crx_bdchbohmgmandnkikanoblpemggpldff.Chromium"
)
...
# Match the existing Chromium app wall layout: slight bleed beyond the screen
# edges to hide borders and keep the four apps tightly stitched together.
BLEED_X=16
TOP_BLEED_Y=10
...
echo "Arranging world situation monitor windows on DISPLAY=$DISPLAY_VALUE (${SCREEN_W}x${SCREEN_H})"

move_window "${WINDOW_IDS[monitor-the-situation]}" "$LEFT_X"  "$TOP_Y"    "$WINDOW_W" "$TOP_H"
move_window "${WINDOW_IDS[marinetraffic-com]}" "$LEFT_X"  "$BOTTOM_Y" "$WINDOW_W" "$BOTTOM_H"
move_window "${WINDOW_IDS[glint-trade]}"  "$RIGHT_X" "$TOP_Y"    "$WINDOW_W" "$TOP_H"
move_window "${WINDOW_IDS[flightradar24-com]}" "$RIGHT_X" "$BOTTOM_Y" "$WINDOW_W" "$BOTTOM_H"

restore_window_to_front "${WINDOW_IDS[monitor-the-situation]}"
restore_window_to_front "${WINDOW_IDS[marinetraffic-com]}"
restore_window_to_front "${WINDOW_IDS[glint-trade]}"
restore_window_to_front "${WINDOW_IDS[flightradar24-com]}"
