    ...
    for source_root in source_roots:
        ...
        for skill_dir in sorted(source_root.iterdir()):
            if not skill_dir.is_dir() or not (skill_dir / "SKILL.md").exists():
                continue
            if skill_dir.name == "skill-syncer":
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Sync skills from other agent folders into .gemini/skills")
    parser.add_argument(
        "--source-root",
        action="append",
        default=[],
    ...
    parser.add_argument(
        "--target-root",
        default=str(workspace_root / ".gemini" / "skills"),
        help="Gemini skill target root.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
