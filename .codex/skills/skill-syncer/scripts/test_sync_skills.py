...
class SyncSkillsTest(unittest.TestCase):
    ...
    def test_copy_new_skill_and_rewrite_agent_paths(self):
        ...
        report = self.module.sync_skill_trees(
            [self.claude_root, self.gemini_root],
            self.codex_root,
            dry_run=False,
        )

        self.assertEqual(report["copied"], ["demo"])
        copied = (self.codex_root / "demo" / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn(".codex/skills/demo", copied)
    ...
    def test_update_existing_skill_when_source_is_newer(self):
        ...
        updated = (self.codex_root / "demo" / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("description: new", updated)
        self.assertIn(".codex/skills/demo", updated)

    def test_skip_when_codex_is_newer(self):
        old = time.time() - 100
        new = time.time()
        self.create_skill(
            self.codex_root,
        ...
        current = (self.codex_root / "demo" / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("description: codex", current)
    ...
    def test_dry_run_reports_without_writing(self):
        ...
        self.assertEqual(report["copied"], ["demo"])
        self.assertFalse((self.codex_root / "demo").exists())
