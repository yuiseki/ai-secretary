...
- `whisper.cpp` ビルド済み
  - 例: `repos/_voice/whisper.cpp/build/bin/whisper-server`
- モデル配置済み
  - 例: `repos/_voice/whisper.cpp/models/ggml-small.bin`, `ggml-medium.bin`
- PipeWire / PulseAudio 互換コマンドが使える
  - `pactl`, `parec`
- `tmux` 利用可能
- この環境の operator entrypoint
  - `yuiclaw voice-command operator ...`
- 互換 backend / 実装
  - `yuiclaw voice-command operator`
  - `repos/ahear/python/src/ahear/whisper_listener.py`
  - `repos/arouter/scripts/voice_command_runtime.py`
...
- `whisper-server` モデル既定: `ggml-small.bin`
  - backend: `yuiclaw voice-command operator`
- 言語: `ja`
- `voice command loop`:
...
```bash
# small（速め、現在の既定）
WHISPER_SERVER_MODEL=/home/yuiseki/Workspaces/repos/_voice/whisper.cpp/models/ggml-small.bin \
yuiclaw voice-command operator restart-agent-all

# medium（精度寄り、遅め）
WHISPER_SERVER_MODEL=/home/yuiseki/Workspaces/repos/_voice/whisper.cpp/models/ggml-medium.bin \
yuiclaw voice-command operator restart-agent-all
```
...
```bash
yuiclaw voice-command operator status
yuiclaw voice-command operator logs-server | rg 'loading model from|type\\s+=\\s+'
```
...
```bash
yuiclaw voice-command operator logs-agent | rg '^\\[.*\\] source='
```
...
音声コマンド実装は `repos/arouter/scripts/voice_command_runtime.py` の `parse_command()` にある。
...
- operator entrypoint: `yuiclaw voice-command operator ...`
- 互換 backend: `yuiclaw voice-command operator`
- listen-only 実装: `repos/ahear/python/src/ahear/whisper_listener.py`
