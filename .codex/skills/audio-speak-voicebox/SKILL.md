...
description: "この KDE Plasma / PipeWire 環境で VOICEBOX（VOICEVOX）を使ってオーナーに音声で話しかける。VOICEVOX API(50021) でWAV合成し、repos/acaption の IPC 経由で音声+字幕を同期再生する。必要時のみ paplay 直再生へフォールバックする。"
---
...
- acaption IPC が未起動（`127.0.0.1:47832` に繋がらない）
- acaption 側の音声出力初期化失敗（`logs-overlay` を確認）
